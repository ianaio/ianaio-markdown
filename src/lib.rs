...
// cargo run -p example-markdown --bin ianaio_markdown_test_server -- examples/markdown/dist
//
...
"#;
    ...
    async fn markdown_worker_works() {
        let mut bridge =
            MarkdownWorker::spawner().spawn("http://127.0.0.1:9999/ianaio_markdown_worker.js");
